...
execute as @s[tag=!set] run item replace entity @s weapon.mainhand with diamond_axe
tag @s add set

#only 1
execute if entity @e[type=#zero:zombie,tag=boss,distance=1..256] run tag @s add tank
execute if entity @e[type=#zero:zombie,tag=boss,distance=1..256] run tag @s remove set
execute if entity @e[type=#zero:zombie,tag=boss,distance=1..256] run tag @s remove boss
