...
#swarmer spawner
scoreboard players add @s zero.age 12
execute at @e[type=husk,tag=swarmer,distance=..16] run scoreboard players remove @s zero.age 1
execute if score @s zero.age matches 600.. run summon silverfish ~ ~ ~ {Tags:[checked,set,swarmer]}
execute if score @s zero.age matches 600.. run scoreboard players set @s zero.age 0
